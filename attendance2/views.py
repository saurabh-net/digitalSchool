...
from mywrapper.models import Grade
...
@login_required
def index(request):
	latest_class_list = Grade.objects.all()	
	classLinks =  [p.fullGradeID for p in latest_class_list]
	...
	return render(request,'attendance2/list.html',context)	
...
def viewAttendance(request,classSection):
	...
	return render(request,'attendance2/viewAttendance.html',context)
...
def enterAttendance(request,classSection):
	...
	if request.method == 'POST':
		...
		# return render(request,'attendance2/list.html',{'alert':'alert',"classSection":classSection,'classLinks':classLinks,'classNames':classNames,'zipped_data':zipped_data})
	else:	
		neatSection = classSection
		strippedSection = classSection
		context = {'classSection':classSection,'neatSection':neatSection,'strippedSection':strippedSection,'studentList':studentList}
		# return redirect(reverse('attendance:index'), {"alert":"alert"})
		return render(request,'attendance2/enterAttendance.html',context)
...
def uploadAttendance(request,classSection):
	...
	return render_to_response(
		'attendance2/uploadAttendance.html',
		{
			'form': form,
